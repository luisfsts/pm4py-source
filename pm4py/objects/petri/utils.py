...
def construct_base_event_net(trace_net, instances):
    artificial_start_transition = add_transition(trace_net)
    artificial_end_transition = add_transition(trace_net)
    ...
    add_arc_from_to(place, artificial_start_transition, trace_net)

    i = 1
    for index, instance in enumerate(instances):
        place = petri.petrinet.PetriNet.Place('p_'+ str(i))
        trace_net.places.add(place)
        add_arc_from_to(artificial_start_transition, place , trace_net)
        ...
        add_arc_from_to(place, transition, trace_net)
        place = petri.petrinet.PetriNet.Place('p_'+ str(i+1))
        trace_net.places.add(place)
        add_arc_from_to(transition, place , trace_net)
        add_arc_from_to(place, artificial_end_transition, trace_net)
        i = i + 2

    place = petri.petrinet.PetriNet.Place('final')
    trace_net.places.add(place)
    add_arc_from_to(artificial_end_transition, place, trace_net) 
...
def add_connections(instance, trace_net, instances):
    transition = get_transition_by_name(trace_net, 't_' + instance["name"] + '_' + str(instances.index(instance)))
    ...
    for post_instance in post_instances:
        if not overlap(instance, post_instance):
            post_transition = get_transition_by_name(trace_net, 't_' + post_instance["name"] + '_' + str(instances.index(post_instance)))
            place = petri.petrinet.PetriNet.Place('p_'+ str(len(trace_net.places)))
            trace_net.places.add(place)
            add_arc_from_to(transition, place, trace_net)
            add_arc_from_to(place, post_transition, trace_net)
...
def construct_partially_ordered_trace_net(trace, trace_name_key=xes_util.DEFAULT_NAME_KEY, activity_key=xes_util.DEFAULT_NAME_KEY):
    ...
    return trace_net, initial_marking, final_marking
...
def construct_partially_ordered_trace_net_cost_aware(trace, costs, trace_name_key=xes_util.DEFAULT_NAME_KEY, activity_key=xes_util.DEFAULT_NAME_KEY):
    trace_net = net = petri.petrinet.PetriNet(
        'trace net of %s' % trace.attributes[trace_name_key] if trace_name_key in trace.attributes else ' ')
